...
func NewService(ctx context.Context, cfg *config.Config, logger *logger.Logger, storage interfaces.Storage, leaderSelector LeaderSelector) (Service, error) {
	...
	case config.ShardingModeStandalone:
		roundManager, err := round.NewRoundManager(ctx, cfg, logger, storage, leaderSelector, nil)
		if err != nil {
			return nil, fmt.Errorf("failed to create round manager for standalone mode: %w", err)
		}
	...
	case config.ShardingModeChild:
		...
		roundManager, err := round.NewRoundManager(ctx, cfg, logger, storage, leaderSelector, rootAggregatorClient)
		if err != nil {
